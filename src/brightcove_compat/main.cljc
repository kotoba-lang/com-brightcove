(ns brightcove-compat.main
  "Kotodama WASM entrypoint for the Brightcove clean-room actor (L4) — Clojure port.
  ...
  facts (`brightcove.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "brightcove")
(def tier "L4")
...
(def entity-specs
  [{:entity "Asset"        :plural "assets"        :id-prefix "brightco_ass"
    :fields [:title :type :durationMs :contentRef]   :required [:title :type]
    :coerce {:durationMs :int}          :refs {}}
   {:entity "Rendition"    :plural "renditions"    :id-prefix "brightco_ren"
    :fields [:assetId :format :bitrate :contentRef]  :required [:format :bitrate]
    :coerce {:bitrate :int}             :refs {:assetId "Asset"}}
   {:entity "Channel"      :plural "channels"      :id-prefix "brightco_cha"
    :fields [:name :description :public]             :required [:name :description]
    :coerce {:public :bool}             :refs {}}
   {:entity "Playlist"     :plural "playlists"     :id-prefix "brightco_pla"
    :fields [:channelId :title :itemCount]           :required [:title :itemCount]
    :coerce {:itemCount :int}           :refs {:channelId "Channel"}}
   {:entity "ContentEntry" :plural "contententries" :id-prefix "brightco_con"
    :fields [:modelName :title :locale :published]   :required [:modelName :title]
    :coerce {:published :bool}          :refs {}}
   {:entity "Experiment"   :plural "experiments"   :id-prefix "brightco_exp"
    :fields [:name :variant :conversionRate]         :required [:name :variant]
...
(defn emit-facts
  "EAVT facts for one record: {\"brightcove.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "brightcove-compat" :tier tier :entities entities} 200])
...
(defn start! [] :brightcove-compat/ready)
